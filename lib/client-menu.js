window.__ModuleLoader__.load({
  ...
  factory: () => {
    ...
    // ── desktop pin extension (registered into the official registry) ──
    const pinState = { ids: [] }
    let pinEntry = null
    function setPinMenuLabel(session) {
      if (!pinEntry) return
      const pinned = pinState.ids.includes(session && session.id)
      pinEntry.label = pinned ? '取消置顶' : '置顶会话'
    }
    function registerPinExtension(registry) {
      if (pinEntry) return
      pinEntry = {
        id: 'dsh-desktop.pin',
        order: 80,
        label: '置顶会话',
        visible: (ctx) => !!(ctx && ctx.session),
        run: (ctx) => {
          const session = ctx && ctx.session
          if (!session) return
          const pinned = pinState.ids.includes(session.id)
          // Toggle via a DOM event the desktop shell listens for
          window.dispatchEvent(new CustomEvent('dsh:desktop-pin', { detail: { sessionId: session.id, title: session.displayTitle || session.title || '', toggle: true } }))
          if (pinned) pinState.ids = pinState.ids.filter((id) => id !== session.id)
          else pinState.ids.push(session.id)
          setPinMenuLabel(session)
        },
      }
      registry.register(pinEntry)
    }
    if (apply) {
      const __origApply = apply
      apply = function (ctx) {
        const result = __origApply(ctx)
        registerPinExtension(registry())
        ctx.on?.('dispose', () => { pinEntry = null })
        return result
      }
    }
